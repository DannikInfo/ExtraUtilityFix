...
public class GuiTransferNode extends GuiContainer
{
    ...
    public GuiTransferNode(final IInventory player, final TileEntityTransferNode node) {
        ...
        this.player = player;
        this.xSize = 176;
        if(node instanceof TileEntityTransferNodeEnergy)//fix glitch
        	this.ySize = 84;
        else
        	this.ySize = 222;
    }
    ...
    protected void drawGuiContainerBackgroundLayer(final float f, final int i, final int j) {
        GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        if(node instanceof TileEntityTransferNodeEnergy)//fix glitch
        	this.mc.renderEngine.bindTexture(GuiTransferNode.textureEnergy);
        else
    ...
    static {
        ...
        textureEnergy = new ResourceLocation("extrautils", "textures/guiTransferNodeEnergy.png");//fix glitch
    }
}
