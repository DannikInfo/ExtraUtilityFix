	    ...
	    /*if(this.isPowered()){ //add reverse convert energy.. NOT UNCOMENTED IT!!! IT.. IT.. IT VERY VERY VERY BUGED!!!
	        if (this.clientele.size() > 0) {
	            ...
	            for (final EnergyPosition client : this.clientele) {
	                ...
	                final TileEntity tile = this.worldObj.getTileEntity(x, y, z);
	                final ForgeDirection from = ForgeDirection.getOrientation((int)client.side);
		            if(tile instanceof IEnergySink && this.powerHandler.getEnergyStored() > 4 && !(tile instanceof IEnergyStorage)){
			            final IEnergySink machine = (IEnergySink)tile;
			            int a2 = (int) machine.injectEnergy(from, toIC2(this.powerHandler.getEnergyStored()), toIC2(this.powerHandler.getEnergyStored()));
			            this.powerHandler.extractEnergy((int) toIC2(this.powerHandler.getEnergyStored()), false);
		            }
		            if(tile instanceof IEnergyConductor) {
		            	final IEnergyConductor cable = (IEnergyConductor)tile;
		            	cable.emitsEnergyTo(this, from);
		            }
	            }
	        }
    	}*/
        ...
        if (pipeBlock != null) {
            for (int i = 0; i < 6; ++i) {
                if (pipeBlock.shouldConnectToTile((IBlockAccess)this.worldObj, x, y, z, ForgeDirection.getOrientation(i)) || (pipeBlock instanceof INode && ((INode)pipeBlock).getNodeDir() == ForgeDirection.getOrientation(i))) {
                   ...
                   /* if (TNHelper.isIC2Energy(tile, ForgeDirection.getOrientation(i).getOpposite()) && isPowered()) { //add in list Machines from IC2
                        final EnergyPosition pos = new EnergyPosition(this.pipe_x + Facing.offsetsXForSide[i], this.pipe_y + Facing.offsetsYForSide[i], this.pipe_z + Facing.offsetsZForSide[i], (byte)Facing.oppositeSide[i], "Energy_Extract".equals(pipeBlock.getPipeType()));
                        if (!this.clientele_temp.contains(pos)) {
                            this.clientele_temp.add(pos);
                        }
